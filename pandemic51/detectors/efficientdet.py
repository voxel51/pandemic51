...
class EfficientDet(etal.ObjectDetector, etat.UsesTFSession):
    ...
    def _detect(self, img):
        ...
        for detection in detections:
            ...
            if (class_id in self._labels_map and
                    score > self.config.confidence_thresh):
                objects.add(
                    _to_detected_object(
...
def _to_detected_object(box, score, class_id, img, labels_map):
    ...
    bounding_box = BoundingBox.from_abs_coords(
        box[0], box[1], box[0] + box[2], box[1] + box[3], img=img)

    return DetectedObject(label, bounding_box, confidence=score)
